      ...
      body: ListView(
        ...
        children: [
          FluidGrid<String>(
            ...
            idOf: (item) => item,
            sections: [
              GridSection(id: 's', items: [for (var i = 0; i < widget.itemCount; i++) 'item$i']),
            ],
...
void main() {
  ...
  testWidgets('pinching in raises the column count', (tester) async {
    ...
    await pinch(tester, center: const Offset(200, 200), fromSeparation: 240, toSeparation: 60);

    expect(reported, greaterThan(2));
  });

  testWidgets('a tiny pinch that resolves back to the base count reports nothing', (tester) async {
    int? reported;
    await tester.pumpWidget(_Harness(onCountChanged: (c) => reported = c));
    await tester.pumpAndSettle();

    // Barely change the separation: resolves back to 2.
    await pinch(tester, center: const Offset(200, 200), fromSeparation: 150, toSeparation: 160);

    expect(reported, isNull);
  });

  testWidgets('the resolved count echoes back without a layout jump', (tester) async {
  ...
  testWidgets('a one-finger drag still scrolls the list instead of zooming', (tester) async {
    int? reported;
    final controller = ScrollController();
    addTearDown(controller.dispose);

    await tester.pumpWidget(_Harness(itemCount: 40, onCountChanged: (c) => reported = c, controller: controller));
    await tester.pumpAndSettle();
  ...
  testWidgets('a two-finger pinch does not start a reorder', (tester) async {
    ...
    await tester.pumpWidget(_Harness(reorderEnabled: true, onReorderStarted: started.add));
    await tester.pumpAndSettle();

    await pinch(tester, center: const Offset(200, 200), fromSeparation: 100, toSeparation: 220);

    expect(started, isEmpty);
  });

  testWidgets('re-pinching while the previous settle runs does not snap the grid', (tester) async {
    await tester.pumpWidget(const _Harness());
    await tester.pumpAndSettle();

    // First pinch with sideways travel, released mid-flight.
    const center = Offset(200, 260);
    var g1 = await tester.startGesture(center - const Offset(60, 0), pointer: 1);
    var g2 = await tester.startGesture(center + const Offset(60, 0), pointer: 2);
    for (var step = 1; step <= 3; step++) {
      final drift = Offset(step * 20.0, 0);
      await g1.moveTo(center - const Offset(90, 0) - Offset(step * 12.0, 0) + drift);
      await g2.moveTo(center + const Offset(90, 0) + Offset(step * 12.0, 0) + drift);
      await tester.pump(const Duration(milliseconds: 16));
    }
    await g1.up();
    await g2.up();
  ...
  testWidgets('a mid-settle re-pinch centred somewhere else does not jump the canvas', (tester) async {
    await tester.pumpWidget(const _Harness());
    await tester.pumpAndSettle();
    ...
    var g1 = await tester.startGesture(firstCenter - const Offset(60, 0), pointer: 1);
    var g2 = await tester.startGesture(firstCenter + const Offset(60, 0), pointer: 2);
    for (var step = 1; step <= 3; step++) {
    ...
    final probe = find.byType(_Card).first;
    var previous = tester.getTopLeft(probe);
    g1 = await tester.startGesture(secondCenter - const Offset(60, 0), pointer: 3);
    g2 = await tester.startGesture(secondCenter + const Offset(60, 0), pointer: 4);
    for (var step = 1; step <= 4; step++) {
  ...
  testWidgets('releasing a sideways-drifting pinch adds no horizontal motion', (tester) async {
    // An explicit rubber-band factor keeps this regression exercising the
    // rubber zone (the default is a hard stop).
    await tester.pumpWidget(const _Harness(config: GridZoomConfig(rubberBandFactor: 0.15)));
    await tester.pumpAndSettle();
    ...
    var previous = [for (var i = 0; i < probes.evaluate().length; i++) tester.getTopLeft(probes.at(i))];
    for (var frame = 0; frame < 30; frame++) {
      await tester.pump(const Duration(milliseconds: 16));
      final current = [for (var i = 0; i < probes.evaluate().length; i++) tester.getTopLeft(probes.at(i))];
