...
void main() {
  testWidgets('crossing an integer count never jumps the scroll (rollover flash)', (tester) async {
    final controller = ScrollController();
    addTearDown(controller.dispose);

    await tester.pumpWidget(_Harness(itemCount: 40, square: true, initialCount: 4, controller: controller));
    ...
    final g1 = await tester.startGesture(center - const Offset(baseSeparation / 2, 0), pointer: 1);
    final g2 = await tester.startGesture(center + const Offset(baseSeparation / 2, 0), pointer: 2);
  ...
  testWidgets('the settle glides the pinned point to rest without jiggle or snap', (tester) async {
    final controller = ScrollController();
    addTearDown(controller.dispose);

    await tester.pumpWidget(_Harness(itemCount: 40, square: true, initialCount: 3, controller: controller));
    await tester.pumpAndSettle();
    controller.jumpTo(600);
    await tester.pumpAndSettle();
    ...
    expect(box.animator.zoomActive, isTrue, reason: 'the release left a real settle');
    ...
    double pinnedDy() {
      ...
      return box.localToGlobal(Offset(0, rect.top + box.zoomAnchorFraction.dy * rect.height)).dy;
    }
    ...
    while (box.animator.zoomActive && frames < 300) {
      await tester.pump(const Duration(milliseconds: 16));
      frames++;
      expect(pinError(), lessThan(3), reason: 'settle frame $frames: the pinned point stays under the focal');
      final dy = pinnedDy();
    ...
    expect(signChanges, lessThanOrEqualTo(1), reason: 'the pinned point glides, never oscillates');
    await tester.pumpAndSettle();
  });

  testWidgets('the scroll is corrected at most once per frame while pinching', (tester) async {
    final controller = ScrollController();
    addTearDown(controller.dispose);
    ...
    expect(spread, lessThan(1.0), reason: 'a held pinch keeps the scroll parked (no alternation)');
  ...
  testWidgets('pinching past the range edge produces no movement at all (hard stop)', (tester) async {
    final controller = ScrollController();
    addTearDown(controller.dispose);

    await tester.pumpWidget(_Harness(itemCount: 20, square: true, initialCount: 2, controller: controller));
    await tester.pumpAndSettle();
    ...
    for (var step = 5; step <= 9; step++) {
      ...
      await tester.pump(const Duration(milliseconds: 16));
      expect(tester.getSize(find.byType(_SquareTile).first).width, moreOrLessEquals(edgeWidth, epsilon: 0.01), reason: 'step $step: tile size pinned at the edge');
      expect(controller.offset, moreOrLessEquals(edgeOffset, epsilon: 0.5), reason: 'step $step: scroll pinned at the edge');
      expect(tester.getTopLeft(find.text('item0').first).dy, moreOrLessEquals(edgeTop.dy, epsilon: 0.5), reason: 'step $step: content pinned at the edge');
    }
    await g1.up();
    await g2.up();
    await tester.pumpAndSettle();
    ...
    expect(tester.getSize(find.byType(_SquareTile).first).width, moreOrLessEquals(800, epsilon: 1));
  });
}
